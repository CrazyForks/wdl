...
2. Owner resolution 会 WATCH owner record、generation key、worker delete lock 和 active worker storage pointer。`whole` delete lock 会拒绝 ownership；`version` lock 仍属于 WATCH snapshot，但不会中断 active storage。这个 WATCH 会阻止 claim 在 whole-worker delete 开始后提交。Renew 会先 pipeline 读取 owner/storage snapshot，再通过 Lua CAS 原子比较 owner 原始字节与 active storage pointer，然后刷新 TTL；generation fence 已包含在 owner record 中，不需要再次读取 generation key。
