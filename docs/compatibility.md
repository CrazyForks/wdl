...
| WebSocket upgrade | Supported | WebSocket API and 101 response handling inside workerd. | Gateway terminates a local public WebSocket pair and directly forwards to runtime/do-runtime with bounded backend reconnect and client-frame buffering. | WDL optimizes for preserving long-lived gateway-proxied sockets, while Cloudflare can rely on its global edge session model. | Gateway rolling still drops physical client sockets; application-level resume is not implemented. |
