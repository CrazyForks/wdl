...
- Runtime escape 不应自动变成云凭证泄露。Tenant task network namespace 必须阻断 host IMDS 和宽权限 instance credential。
...
| Host/infra | ECS EC2 hosts、IAM roles、IMDS | Operator 控制 | IMDS block、IAM least privilege、ECS Exec policy |
...
EC2-backed ECS capacity 承载平台服务，包括执行 tenant 的 runtime task。Tenant task 不能访问 host instance metadata：

- EC2 launch/user-data 必须阻断 awsvpc task 访问 host IMDS。
- IMDS hop limit 和 ECS agent setting 必须一起 review。
- Instance profile 用于 ECS/SSM/platform operation，不是 tenant application capability。

运行 tenant code 的 runtime service 与平台服务共享 EC2 capacity，因此 IMDS blocking、public-only outbound binding 和最小权限 task role 都是 tenant isolation contract 的一部分。ECS Exec 只应在需要 platform operator access 的地方启用。
...
- `tests/unit/style-contracts.test.js`：route channel、hidden Fetcher stripping、internal socket split、IMDS block、low-cardinality metrics 等 drift guard。
