...
| WebSocket upgrade | Supported | workerd 内的 WebSocket API 和 101 response 处理。 | Gateway 在本地终结公开 WebSocket pair，并通过有界 backend reconnect 和 client-frame buffer 直接转发到 runtime/do-runtime。 | WDL 优先保留 gateway-proxied 长连接；Cloudflare 依赖自己的全球 edge session 模型。 | Gateway rolling 仍会断开物理 client socket；应用级 resume 未实现。 |
