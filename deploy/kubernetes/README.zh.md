...
- `overlays/local/` 提供本地 smoke secret、namespace 和本地存储默认值。
- `overlays/local-gateway/` 为本地主机名增加 Gateway API 资源。
- `overlays/local-metadata-guard/` 应用本地 overlay，并额外加入共享的 metadata-guard component。
- `overlays/local-gateway-metadata-guard/` 应用本地 gateway overlay，并额外加入同一个 component。
- `storage/local-nfs/` 包含一个本地 ReadWriteMany provisioner 的 Helm values，可在开发集群中模拟云厂商 RWX 存储。

base 和本地 overlay 使用发布镜像契约：workerd 服务拉取 `docker.io/getwdl/wdl-workerd:latest`，Rust sidecar 和 Rust service 拉取 `docker.io/getwdl/wdl-rust:latest`。WDL container 显式设置 `imagePullPolicy: Always`，因此 apply 这套栈不依赖开发机本地构建或手工导入的镜像。
...
如果本地或 staging 集群只需要最小的云 metadata 防护、但暂时不做完整生产 egress allowlist，可以用 `overlays/local-metadata-guard/` 代替 `overlays/local/`，或用 `overlays/local-gateway-metadata-guard/` 代替 `overlays/local-gateway/`。这些 overlay 会为 WDL Pod 加一条 egress NetworkPolicy：允许普通 IPv4 出站，但排除常见 metadata endpoint `169.254.169.254/32` 和 `169.254.170.2/32`。这不是完整生产 egress 策略；生产 overlay 仍需要按具体云厂商决定 DNS、对象存储、托管 Valkey、admin endpoint 和 IPv6 metadata 的访问规则。
...
这是推荐的本地 smoke 路径。它已在 Docker Desktop Kubernetes 上验证，该环境会把 Gateway `LoadBalancer` 发布到 `127.0.0.1`。其他 Gateway API 集群可以沿用相同形态，但必须把 smoke hostname 映射到实际的外部 Gateway 地址，或通过 NodePort、本地 tunnel 暴露入口。

1. 启动或连接到一个本地 Kubernetes 集群。
2. 按上一节安装本地 NFS provisioner；如果你的集群已经提供名为 `wdl-nfs-rwx` 的 RWX storage class，可以跳过。
3. 安装 Gateway API standard CRD 和 NGINX Gateway Fabric。本地 overlay 固定使用 NGINX Gateway Fabric 2.6.7：
...
5. 应用 Gateway API overlay。它会包含本地 base，并把 `ASSETS_CDN_BASE` patch 为 `http://s3mock.local/wdl-assets`：

   ```bash
   kubectl apply -k deploy/kubernetes/overlays/local-gateway
   ```

6. 等待 Gateway 和所有 WDL workload ready：
...
7. 在接受 tenant ASSETS 部署前，先 bootstrap 永久的 `__system__/s3-cleanup` worker：
   ...
   这个 worker 是 ASSETS 生命周期的必要组件。缺少它时，version 和 worker delete 仍能 enqueue cleanup intent，但没有 consumer 会删除 S3 object。
...
greenfield 栈 ready 后，建议通过 Gateway API hostnames 跑几条租户级检查。这些检查比 Pod Ready 更能说明业务链路完整。
...
HTML 中应包含 `http://s3mock.local/wdl-assets/...` 下的 URL。再直接访问其中一个 URL，确认对象存储路径也能通过 Gateway 访问。

随后删除这个测试 Worker，并确认 system cleanup worker 会移除其 ASSETS object：
...
如果不想安装 Gateway API 实现，可以应用 local overlay 并转发面向 host 的端口：
...
如果已经安装 NGINX Gateway Fabric，可以直接应用 Gateway API overlay：
...
本地 Gateway API overlay 期望存在 `nginx` GatewayClass，并路由：
...
这个 overlay 会把 `assets-proxy` 的 NetworkPolicy 只开放给 `wdl` Gateway 的 data-plane Pod，并且只把 s3mock 开给 WDL runtime Pod 和这个 proxy。它不会通过 Gateway 暴露完整 s3mock S3 API。如果使用其他 Gateway API 实现，需要在 `overlays/local-gateway/gateway-network-policy.yaml` 中提供等价的 data-plane Pod selector。
